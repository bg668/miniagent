...
- 支持流式输出（文本与 thinking 分区显示）
- 支持多轮对话（保留上下文）
- 配置从 examples/config.json 读取（模型、base_url、system_prompt、extra_body 等）
...
from paimonsdk import Agent, AgentOptions, AssistantMessage, ModelInfo, TextContent, ThinkingContent, ThinkingLevel
...
def create_thinking_level(config: dict) -> ThinkingLevel:
    """从配置创建 ThinkingLevel；默认 OFF 表示不传递给请求"""
    raw_value = config.get("thinking_level")
    if raw_value is None:
        return ThinkingLevel.OFF
    return ThinkingLevel(str(raw_value).lower())
...
async def main():
    ...
    current_thinking_length = 0
    answer_started = False
    thinking_started = False

    def on_event(event, cancel_token):
        """处理 Agent 事件，实现打字机效果"""
        nonlocal current_text_length, current_thinking_length, answer_started, thinking_started
        ...
        # 消息更新：打印新增的内容
            ...
            for block in message.content:
                if isinstance(block, ThinkingContent):
                    ...
                    if len(full_thinking) > current_thinking_length:
                        if not thinking_started:
                            print("🧠 Thinking: ", end="", flush=True)
                            thinking_started = True
                        new_thinking = full_thinking[current_thinking_length:]
                        print(new_thinking, end="", flush=True)
                        current_thinking_length = len(full_thinking)
