...
| 配置项 | 当前作用 |
| --- | --- |
| `system_prompt` | 保存系统提示词文本，但当前不会自动注入 OpenAI 请求 |
| `model` | 声明当前模型元信息 |
| `tools` | 注册可执行工具列表 |
...
### 16.1 `system_prompt` 当前不会自动进入 OpenAI 请求

虽然 `system_prompt` 在状态和配置中都有定义，但默认 `OpenAIChatCompletionsAdapter` 在构造请求时并不会自动把它映射成 OpenAI 的 system message。因此，在当前默认链路下，`system_prompt` 仍属于已建模但未默认注入请求的字段。
...
当前 `agentsdk/adapters/openai_chatcompletions.py` 的导入路径已经与 `agentsdk` 包名保持一致。在当前仓库执行 `uv run pytest agentsdk/tests -q` 时，现有 12 项测试可以通过。
